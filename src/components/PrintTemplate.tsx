...
export default function PrintTemplate({ profile, onPrint }: PrintTemplateProps) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const opt = {
        ...
        filename: `${profile.reg_no || profile.id || 'profile'}.pdf`,
  ...
  const handleDownloadWord = async () => {
    try {
      await downloadProfileDocx(profile, `${profile.reg_no || profile.id || 'profile'}.docx`);
    } catch (error) {
      ...
      <div ref={printRef} className="print-content">
        ...
        <div className="print-body">
          <table className="profile-table">
            ...
            <tbody>
              <tr>
                <td className="username-cell" rowSpan={24}>{regNo}</td>
